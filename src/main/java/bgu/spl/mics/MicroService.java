...
public abstract class MicroService implements Runnable {
    ...
    /**
     * @return true if this service received the terminate event (i.e. a
     *         broadcast message of type {@link Terminate})
     */
...
public final void run() {
    try {
        // רישום השירות ל-MessageBus
        MessageBusImpl.getInstance().register(this);
        // אתחול השירות (רישום ל-Events ול-Broadcasts)
        initialize();
        // לולאת עיבוד הודעות
        while (!terminated) {
            try {
                // קבלת הודעה מה-MessageBus
                Message message = MessageBusImpl.getInstance().awaitMessage(this);
                // עיבוד ההודעה באמצעות ה-Callback המתאים
                if (message != null) {
                ...
                Thread.currentThread().interrupt(); // סימון ה-Thread כ-Marked for termination
                
            } catch (Exception e) {
                e.printStackTrace(); // טיפול בחריגות אחרות
            }
        }
    } finally {
        // ביטול רישום השירות מה-MessageBus
